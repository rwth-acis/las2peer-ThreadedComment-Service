...
export SERVICE_VERSION2=$(awk -F "=" '/service2.version/ {print $2}' etc/ant_configuration/service.properties)
export SERVICE_NAME2=$(awk -F "=" '/service2.name/ {print $2}' etc/ant_configuration/service.properties)
export SERVICE_CLASS2=$(awk -F "=" '/service2.class/ {print $2}' etc/ant_configuration/service.properties)
export SERVICE2=${SERVICE_NAME2}.${SERVICE_CLASS2}@${SERVICE_VERSION2}
...
if [[ -z "${@}" ]]
then
  exec ${LAUNCH_COMMAND} --observer startService\("'""${SERVICE}""'", "'"someNewPass"'"\) startService\("'""${SERVICE2}""'", "'"someNewPass"'"\) startWebConnector
else
  exec ${LAUNCH_COMMAND} ${@}
fi
